...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    const applyTheme = () => {
      ...
      if (isControlled) {
        // A controlled parent owns the theme marker and applies it inside the
        // view transition's update callback (Kivo writes `data-theme`).
        onThemeChange?.(newTheme ? "dark" : "light")
      } else {
        // Uncontrolled mode drives a class-based theme, so the class has to flip
        // synchronously inside the callback for the snapshot to see the new theme.
