...
export type FolderFloatItem = { label: string; value: string; dock?: boolean }
export type FolderFloatTrigger = 'hover' | 'click'

export type FolderFloatProps = {
  items?: Array<string | FolderFloatItem>
  label?: string
  sublabel?: string
...
type MeasuredSize = { w: number; h: number }
type LayoutPosition = { x: number; y: number; r: number }
...
type PhysicsZone = { left: number; right: number; top: number; bottom: number }
...
const PAD = 28
const CHAR = 6.8
// The fan keeps the pills apart: GAP between two pills on a row, ROW between two
// rows. ROW stays modest because the whole stack has to fit the grid's top pad.
const GAP = 20
const ROW = 52
const DRAG_MIN = 4
const ZONE_PAD = 8
...
const layout = (
  ...
  let width = 0
  // Docked items skip the fan and land on the folder's own face, one row below
  // the lowest fan row, so a long list costs no extra height above the folder.
  const docked: number[] = []
  list.forEach((item, i) => {
    if (item.dock) {
      docked.push(i)
  ...
  rows.forEach((r, ri) => {
    let x = -r.width / 2
    // The rows lean against each other, but not far enough to push a pill past
    // the edge of a narrow card.
    const shift = (ri % 2 ? 1 : -1) * Math.min(8, spread * 0.06)
    r.items.forEach(({ i, pw }) => {
      const j = jitter(i)
      pos[i] = { x: x + pw / 2 + shift + (j - 0.5) * 4, y: -lift - ri * ROW - j * 6, r: tilt * (j * 2 - 1) }
      x += pw + GAP
    })
  })
  docked.forEach((i) => {
    const j = jitter(i)
    pos[i] = { x: (j - 0.5) * 4, y: ROW - lift - j * 6, r: tilt * 0.5 * (j * 2 - 1) }
  })
  return pos
}
...
export default function FolderFloat({
  ...
  const [sizes, setSizes] = useState<MeasuredSize[]>([])
  const anchorRef = useRef<HTMLDivElement>(null)
  const pillRefs = useRef<Array<HTMLButtonElement | null>>([])
  const world = useRef<PhysicsWorld>({
    engine: null,
    bodies: [],
    sizes: [],
    raf: 0,
  ...
  const latest = useRef<LatestConfig>({ drift, reduce: false })
  ...
  useLayoutEffect(() => {
    const measure = () => {
      const measured = pillRefs.current
        .slice(0, n)
        .map((el) => (el ? { w: el.offsetWidth, h: el.offsetHeight } : null))
      if (measured.some((s) => !s)) return
      const next = measured as MeasuredSize[]
      setSizes((prev) =>
  ...
  const startPhysics = useCallback(() => {
    ...
    const pills = els as HTMLButtonElement[]
    const engine = Engine.create({ gravity: { x: 0, y: 0 } })
    engine.enableSleeping = false
    w.engine = engine
    w.sizes = pills.map((el) => ({ w: el.offsetWidth, h: el.offsetHeight }))
  ...
  const pointerAt = (e: { clientX: number; clientY: number }) => {
  ...
  const up = (e: ReactPointerEvent<HTMLButtonElement>, i: number, item: FolderFloatItem) => {
    const w = world.current
    const d = w.drag
    if (!d || d.i !== i || d.id !== e.pointerId) return
    w.drag = null
